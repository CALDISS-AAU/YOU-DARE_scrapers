...
class DynamicSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    ...
    start_urls = ['https://mallarduk.com/category/comment/', 'https://mallarduk.com/category/culture/'] # The url where the content to be scraped is found - can be multiple urls IF THE CSS/XPATH IS IDENTICAL!
    ...
    external_links_XPATH = '//div[@class="progression-studios-blog-single-excerpt"]//@href'
    article_categories = '//div[@class="tags-progression"]//text()'
    ...
    def parse_article(self, response, article_link): # Can be renamed. IF IT IS REMEBER TO REDIRECT THE CALLBACK IN PARSE!
        ...
        article_categories = response.xpath(self.article_categories).getall()
        article_categories_clean = [c.strip() for c in article_categories if c.strip()]
        ...
        items['categories'] = article_categories_clean
