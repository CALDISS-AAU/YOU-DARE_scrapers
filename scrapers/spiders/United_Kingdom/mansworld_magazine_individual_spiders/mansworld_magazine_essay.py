...
''' To run this spider pass the following to the terminal:
        ...
        scrapy crawl manworlds_magazine_essay_SPIDER -a max_scrolls=x # MUST MATCH SPIDER NAME!
    where -a max_scrolls=x is an optional parameter
    OR
        cd ./YOU-DARE/scrapers
        mkdir -p /work/YOU-DARE/scrapers/data/United_Kingdom/manworlds_magazine_essay_SPIDER # If the folder does not yet exist
        nohup scrapy crawl manworlds_magazine_essay_SPIDER > /work/YOU-DARE/scrapers/data/United_Kingdom/manworlds_magazine_essay_SPIDER/manworlds_magazine_essay_SPIDER_2025-09-10_SPIDER.log
'''
...
class DynamicSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    name = 'manworlds_magazine_essay_SPIDER' # Spider name - must be unique within given project
    ...
    source = 'Mansworld magazine - essay' # The source of the articles - NOT the author!
    start_urls = ['https://mansworldmag.online/category/essay/'] # The url where the content to be scraped is found - can be multiple urls IF THE CSS/XPATH IS IDENTICAL!
