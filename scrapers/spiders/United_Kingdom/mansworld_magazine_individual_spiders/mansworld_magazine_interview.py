...
''' To run this spider pass the following to the terminal:
        ...
        scrapy crawl manworlds_magazine_interview_SPIDER -a max_scrolls=x # MUST MATCH SPIDER NAME!
    where -a max_scrolls=x is an optional parameter
    OR
        cd ./YOU-DARE/scrapers
        mkdir -p /work/YOU-DARE/scrapers/data/United_Kingdom/manworlds_magazine_interview_SPIDER # If the folder does not yet exist
        nohup scrapy crawl manworlds_magazine_interview_SPIDER > /work/YOU-DARE/scrapers/data/United_Kingdom/manworlds_magazine_interview_SPIDER/manworlds_magazine_interview_SPIDER_2025-09-10_SPIDER.log
'''
...
class DynamicSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    name = 'manworlds_magazine_interview_SPIDER' # Spider name - must be unique within given project
    ...
    source = 'Mansworld magazine - interview' # The source of the articles - NOT the author!
    start_urls = ['https://mansworldmag.online/category/interview/'] # The url where the content to be scraped is found - can be multiple urls IF THE CSS/XPATH IS IDENTICAL!
    ...
    '''
    links_to_follow_CSS = 'article a.grve-read-more::attr(href)'
    articles_CSS = 'article a.grve-read-more'
    ...
    '''
    article_title_CSS = 'h2.grve-title *::text'
    author_CSS = '.grve-title.grve-text-primary-1 .grve-clipping-content *::text' #Sketchy since it's based on the colour of the text, but it's the only differentiation between this and category
    publication_date_CSS = 'meta[property="article:published_time"]::attr(content)'
    article_text_bits_CSS = 'article .grve-text p *::text, article .grve-text ul *::text'
    article_categories_CSS = '.grve-title.grve-text-black .grve-clipping-content *::text'
    image_links_CSS = 'article .grve-image-item img::attr(srcset)'
    external_links_CSS = 'article .grve-text p a::attr(href), article .grve-text ul a::attr(href)'
    embedded_media_CSS = 'article .grve-text p iframe::attr(src), article .grve-text ul iframe::attr(src)'
    # ... other queries
