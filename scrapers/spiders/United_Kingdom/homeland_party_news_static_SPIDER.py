...
''' To run this spider pass the following to the terminal:
    ...
    OR
        ...
        mkdir -p ./data/United_Kingdom/homeland_party_news_SPIDER # If the folder does not yet exist
        nohup scrapy crawl homeland_party_news_SPIDER -a max_pages=1 > ./data/United_Kingdom/homeland_party_news_SPIDER/homeland_party_news_SPIDER_$(date +%F).log
'''
...
class StaticSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    ...
    source = 'Homeland party' # The source of the articles - NOT the author!
    ...
    links_in_text_CSS = '.entry-content h5 a::attr(href), .entry-content p a::attr(href)'
    other_items = None
