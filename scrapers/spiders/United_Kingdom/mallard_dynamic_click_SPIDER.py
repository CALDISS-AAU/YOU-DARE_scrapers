...
''' To run this spider pass the following to the terminal:
        ...
        scrapy crawl mallard_dynamic_SPIDER -a max_clicks=x # MUST MATCH SPIDER NAME!
    where -a max_clicks=x is an optional parameter to limit the number of clicks to render more articles on the front page
    OR
        cd ./path/to/YOU-DARE_scrapers-folder
        mkdir -p ./data/United_Kingdom/mallard_dynamic_SPIDER # If the folder does not yet exist
        nohup scrapy crawl mallard_dynamic_SPIDER -a max_clicks=0 > ./data/United_Kingdom/mallard_dynamic_SPIDER/mallard_dynamic_SPIDER_$(date +%F).log
'''
...
class DynamicSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    name = 'mallard_dynamic_SPIDER' # Spider name - must be unique within given project
    ...
    start_urls = [
        'https://mallarduk.com/category/comment/', 
        'https://mallarduk.com/category/culture/'
    ...
    '''
    links_to_follow_CSS = "h2 a::attr(href)"
    click_button_selector_CSS = "ul.page-numbers li a.next" # CSS selector for the "Load more" button
    ...
    '''
    article_title_XPATH = '//*[@id="blog-post-title-meta-container"]/div/h1/text()'
    publication_date_CSS = 'meta[property="article:published_time"]::attr(content)'
    author_XPATH = '/html/body//div[2]/div/div[3]/span[1]/a/text()'
    article_categories_XPATH = '//div[@class="tags-progression"]//text()'
    article_text_XPATH = '//div[@class="progression-studios-blog-single-excerpt"]//text()'
    image_links_XPATH = '//div[@class="progression-studios-blog-single-excerpt"]//img/@src'
    embedded_media_links_XPATH = '//div[contains(@href, "youtube.com")]/@href'
    links_in_text_XPATH = '//div[@class="progression-studios-blog-single-excerpt"]//@href'
    other_items = None
    ...
    def parse_article(self, response, article_link): # Can be renamed. IF IT IS REMEBER TO REDIRECT THE CALLBACK IN PARSE!
        ...
        article_title_clean = response.xpath(self.article_title_XPATH).get() # .get() returns only the first element. Use .getall() to return a list of all elements if more than one element is expected
        # Extract 'publication_date' 
        publication_date = response.css(self.publication_date_CSS).get()
        # Extract 'author' 
        author_clean = response.xpath(self.author_XPATH).getall()
        # Extract 'article_categories' 
        article_categories_raw = response.xpath(self.article_categories_XPATH).getall()
        article_categories = [c.strip() for c in article_categories_raw if c.strip()]
