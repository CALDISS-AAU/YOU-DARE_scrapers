...
''' To run this spider pass the following to the terminal:
        ...
        scrapy crawl manworlds_magazine_FULL_SPIDER -a max_scrolls=x # MUST MATCH SPIDER NAME!
    where -a max_scrolls=x is an optional parameter
    OR
        cd ./YOU-DARE/scrapers
        mkdir -p /work/YOU-DARE/scrapers/data/United_Kingdom/manworlds_magazine_FULL_SPIDER # If the folder does not yet exist
        nohup scrapy crawl manworlds_magazine_FULL_SPIDER > /work/YOU-DARE/scrapers/data/United_Kingdom/manworlds_magazine_FULL_SPIDER/manworlds_magazine_FULL_SPIDER_2025-10-20_SPIDER.log
'''
...
class DynamicSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    name = 'manworlds_magazine_FULL_SPIDER' # Spider name - must be unique within given project
    ...
    source = 'Mansworld magazine' # The source of the articles - NOT the author!
    start_urls = ['https://mansworldmag.online/category/interview/',
                  'https://mansworldmag.online/category/essay/'
    ...
    article_text_bits_CSS = 'article .grve-text p *::text, article .grve-text ul *::text, article .grve-text li *::text'
