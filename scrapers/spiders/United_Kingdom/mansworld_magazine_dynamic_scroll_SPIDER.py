...
''' To run this spider pass the following to the terminal:
        ...
        scrapy crawl manworlds_magazine_SPIDER -a max_scrolls=x # MUST MATCH SPIDER NAME!
    where -a max_scrolls=x is an optional parameter to limit the number of scrolls to render more articles on the front page
    OR
        cd ./path/to/YOU-DARE_scrapers-folder
        mkdir -p ./data/United_Kingdom/manworlds_magazine_SPIDER # If the folder does not yet exist
        nohup scrapy crawl manworlds_magazine_SPIDER -a max_scrolls=0 > ./data/United_Kingdom/manworlds_magazine_SPIDER/manworlds_magazine_SPIDER_$(date +%F).log
'''
...
class DynamicScrollSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    name = 'manworlds_magazine_SPIDER' # Spider name - must be unique within given project
    ...
    start_urls = [
        'https://mansworldmag.online/category/interview/',
    ...
    article_text_CSS = 'article .grve-text p *::text, article .grve-text ul *::text, article .grve-text li *::text'
    image_links_CSS = 'article .grve-image-item img::attr(srcset)'
    embedded_media_links_CSS = 'article .grve-text p iframe::attr(src), article .grve-text ul iframe::attr(src)'
    links_in_text_CSS = 'article .grve-text p a::attr(href), article .grve-text ul a::attr(href)'
    other_items = None
