...
''' To run this spider pass the following to the terminal:
        ...
        scrapy crawl viktor_orban_hirek_SPIDER -a max_clicks=x # MUST MATCH SPIDER NAME!
    where -a max_clicks=x is an optional parameter to limit the number of clicks to render more articles on the front page
    OR
        cd ./path/to/YOU-DARE_scrapers-folder
        mkdir -p ./data/Hungary/viktor_orban_hirek_SPIDER # If the folder does not yet exist
        nohup scrapy crawl viktor_orban_hirek_SPIDER -a max_clicks=0 > ./data/Hungary/viktor_orban_hirek_SPIDER/viktor_orban_hirek_SPIDER_$(date +%F).log
'''
...
class DynamicSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    name = 'viktor_orban_hirek_SPIDER' # Spider name - must be unique within given project
    ...
    source = 'Viktor Orban' # The source of the articles - NOT the author!
    start_urls = ['https://miniszterelnok.hu/hirek/'] # The url where the content to be scraped is found - can be multiple urls IF THE CSS/XPATH IS IDENTICAL!
    ...
    '''
    links_to_follow_XPATH = "//a[contains(@class,'elementor-button')][.//span[contains(normalize-space(),'Elolvasom')]]|//div[contains(@class,'elementor-element')]//a[contains(@class,'jet-listing-dynamic-link__link')]"
    click_button_selector_XPATH = "//a[contains(@class,'elementor-button')][.//span[contains(normalize-space(),'további hírek')]]" # CSS selector for the "Load more" button
    ...
    article_text_CSS = 'div.jet-listing-dynamic-field__content *::text, div.elementor-element.elementor-widget.elementor-widget-text-editor *::text, div#post_content_col_1 *::text'
    ...
    links_in_text_CSS = 'div.jet-listing-dynamic-field__content a::attr(href), div.elementor-element.elementor-widget.elementor-widget-text-editor a::attr(href), div#post_content_col_1 a::attr(href)'
    other_items = None
