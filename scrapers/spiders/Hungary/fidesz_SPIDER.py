...
''' To run this spider pass the following to the terminal:
        ...
        scrapy crawl fidesz_hirek_SPIDER -a max_pages=x # MUST MATCH SPIDER NAME!
    where -a max_pages=x is an optional parameter
    OR
        cd ./YOU-DARE/scrapers
        mkdir -p /work/YOU-DARE/scrapers/data/Hungary/fidesz_hirek_SPIDER # If the folder does not yet exist
        nohup scrapy crawl fidesz_hirek_SPIDER > /work/YOU-DARE/scrapers/data/Hungary/fidesz_hirek_SPIDER/fidesz_hirek_SPIDER_2025-09-03_SPIDER.log
'''
...
class DynamicSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    name = 'fidesz_hirek_SPIDER' # Spider name - must be unique within given project
    ...
    click_button_selector_STOP_CSS = 'li.rc-pagination-next.rc-pagination-disabled > a.rc-pagination-item-link' # CSS for last "next page"-button (it still has a link but is disabled so it would keep loading the same articles on the last page indefinitely)
    ...
    '''
    # article_title_XPATH = '//*[@id="content"]/div/div/div[1]/div[1]/div/div/h2/text()'
    # author_XPATH = '//*[@id="content"]/div/div/div[1]/div[1]/div/div/div[4]/p[10]/text()'
    # publication_date_xpath = '//*[@id="content"]/div/div/div[1]/div[1]/div/div/div[3]/strong/text()'
    # article_text_bits_CSS = '.news__lead *::text, .news__body *::text'
    # image_links_CSS = '.news.news--details img::attr(src)'
    # external_links_CSS = '.news.news--details a::attr(href)'
    # article_HTML_bits_CSS = '.news.news--details'
    # youtube_CSS = '.news.news--details > iframe::attr(src)'
    ...
    def parse_article(self, response, article_link): # Can be renamed. IF IT IS REMEBER TO REDIRECT THE CALLBACK IN PARSE!
        ...
        # Extract 'author' 
        author = self.author #response.xpath(self.author_XPATH).get()
        ...
        # items['article_HTML'] = article_HTML_bits
