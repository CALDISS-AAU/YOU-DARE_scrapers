...
#     name = "fidesz_videos_SPIDER"
#     allowed_domains = ["fidesz.hu"]
#     region = "Hungary"  # harmless if your project expects it somewhere
#     TARGET_PATH = "/work/YOU-DARE/scrapers/data/Hungary/fidesz_videos_SPIDER/data_fidesz_videos_SPIDER.txt"

#     custom_settings = {
#         "FEEDS": {},  # disable feed exporter for this spider (we write our own .txt)
#         "DOWNLOAD_HANDLERS": {
#             "http": "scrapy_playwright.handler.ScrapyPlaywrightDownloadHandler",
#             "https": "scrapy_playwright.handler.ScrapyPlaywrightDownloadHandler",
#         },
...
#     async def start(self):
#         os.makedirs(os.path.dirname(self.TARGET_PATH), exist_ok=True)
#         self.out = open(self.TARGET_PATH, "w", encoding="utf-8")
#         self.seen = set()
#         base = "https://fidesz.hu/videok?page="
...
#         hrefs = response.css(".video-item .video-title a::attr(href)").getall()
#         if not hrefs:
#             hrefs = response.css('a[href^="/videok/"]::attr(href)').getall()
#         seen_local = set()
#         for h in hrefs:
#             if h and h not in seen_local:
#                 seen_local.add(h)
...
#                             PageMethod("wait_for_selector", "div.video--play button"),
#                             PageMethod("click", "div.video--play button"),
#                             PageMethod("wait_for_selector", "iframe[src]", timeout=10000),
#                         ],
#                     },
#                 )
...
#                 pass
#         for raw in srcs or []:
#             url = response.urljoin(raw.strip()) if raw else ""
#             if not url or self._is_facebook(url):
#                 continue
#             if url not in self.seen:
#                 self.seen.add(url)
#                 self.out.write(url + "\n")
