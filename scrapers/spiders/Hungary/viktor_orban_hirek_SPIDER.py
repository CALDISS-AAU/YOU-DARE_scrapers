...
''' To run this spider pass the following to the terminal:
        ...
        scrapy crawl viktor_orban_hirek_SPIDER -a max_pages=x # MUST MATCH SPIDER NAME!
    where -a max_pages=x is an optional parameter
    OR
        cd ./YOU-DARE/scrapers
        nohup scrapy crawl viktor_orban_hirek_SPIDER > /work/YOU-DARE/scrapers/data/Hungary/viktor_orban_hirek_SPIDER/viktor_orban_hirek_2025-08-19_SPIDER.log
'''
...
class DynamicSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    ...
    source = 'Viktor Orban - hirek' # The source of the articles - NOT the author!
