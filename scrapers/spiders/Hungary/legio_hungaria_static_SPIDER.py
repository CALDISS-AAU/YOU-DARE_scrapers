...
''' To run this spider pass the following to the terminal:
    ...
    OR
        ...
        mkdir -p ./data/Hungary/legio_hungaria_static_SPIDER # If the folder does not yet exist
        nohup scrapy crawl legio_hungaria_static_SPIDER -a max_pages=1 > ./data/Hungary/legio_hungaria_static_SPIDER/legio_hungaria_static_SPIDER_$(date +%F).log
'''
...
class StaticSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    ...
    source = 'Legio Hungaria' # The base source name - each keyword will be appended to this
    start_urls = ['https://legiohungaria.org/']  # Template for keyword search URLs
    ...
    author_CSS = None
    article_categories_CSS = '.tags a::text'
    article_text_CSS = '.content.postcontent p *::text'
