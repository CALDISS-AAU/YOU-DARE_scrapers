...
''' To run this spider pass the following to the terminal:
        ...
        scrapy crawl fidesz_SPIDER -a max_clicks=x # MUST MATCH SPIDER NAME!
    where -a max_clicks=x is an optional parameter to limit the number of clicks to render more articles on the front page
    OR
        cd ./path/to/YOU-DARE_scrapers-folder
        mkdir -p ./data/Hungary/fidesz_SPIDER # If the folder does not yet exist
        nohup scrapy crawl fidesz_SPIDER -a max_clicks=0 > ./data/Hungary/fidesz_SPIDER/fidesz_SPIDER_$(date +%F).log
'''
...
class DynamicSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    name = 'fidesz_SPIDER' # Spider name - must be unique within given project
    ...
    source = 'Fidesz' # The source of the articles - NOT the author!
    start_urls = ['https://fidesz.hu/hirek'] # The url where the content to be scraped is found - can be multiple urls IF THE CSS/XPATH IS IDENTICAL!
    ...
    # click_button_selector_CSS = "li.rc-pagination-next > a.rc-pagination-item-link"
    # click_button_selector_STOP_CSS = 'li.rc-pagination-next.rc-pagination-disabled > a.rc-pagination-item-link' # If this is not relevant, set this query to None 
    ...
    article_text_CSS = '.news__lead *::text, .news__body *::text'
    image_links_CSS = '.news.news--details img::attr(src), article img::attr(src)'
    embedded_media_links_CSS = '.news.news--details iframe::attr(src), article iframe::attr(src)'
    links_in_text_CSS = '.news.news--details a::attr(href), article a::attr(href)'
    other_items = None
    ...
    def parse(self, response):
        # Extra functionality to automatically generate front pages since their urls change like one would expect from a static scraper 
        base = response.url

        k = int(self.max_clicks or 0)
        extra_pages = [f"{base}?page={i}" for i in range(2, 2 + k)]
        page_urls = [base] + extra_pages

        seen_links = set()

        for idx_url in page_urls:
            index_html = yield deferToThread(
                asyncio.run,
            ...
            sel = Selector(text=index_html)
            hrefs = sel.css(self.links_to_follow_CSS).getall() or []
            links = [response.urljoin(h) for h in hrefs if h]

            for link in links:
                if link not in self.existing_links and link not in seen_links:
                    seen_links.add(link)

        self.logger.info(f"Found {len(seen_links)} unique article links across {len(page_urls)} page(s)")

        collected_items = []

        for link in seen_links:
            article_html = yield deferToThread(
                asyncio.run,
                Dynamic_Click_Scrapy.fetch_page_with_playwright(
                    link,
                    # timeout_time=90000, # default value = 60000, increase if needed
    ...
    def parse_article(self, response, article_link): # Can be renamed. IF IT IS REMEBER TO REDIRECT THE CALLBACK IN PARSE!
        ...
        # Extract 'author' 
        author = self.author_XPATH #response.xpath(self.author_XPATH).get()
        ...
        article_HTML_bits = response.get() #response.css(self.article_HTML_bits_CSS).getall()
