...
''' To run this spider pass the following to the terminal:
        ...
        scrapy crawl noua_dreapta_opinii_SPIDER -a max_pages=x # MUST MATCH SPIDER NAME!
    ...
    OR
        ...
        mkdir -p ./data/Romania/noua_dreapta_opinii_SPIDER # If the folder does not yet exist
        nohup scrapy crawl noua_dreapta_opinii_SPIDER -a max_pages=1 > ./data/Romania/noua_dreapta_opinii_SPIDER/noua_dreapta_opinii_SPIDER_$(date +%F).log
'''
...
class StaticSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    ...
    start_urls = [
        'https://www.nouadreapta.org/opinii.html', 
        'https://www.nouadreapta.org/opinii.html?start=10'
    ...
    def parse_article(self, response): # Can be renamed. IF IT IS REMEBER TO REDIRECT THE CALLBACK IN PARSE_FRONT!
        ...
        # Extract 'author' 
        author_clean = self.author_XPATH
        ...
        links_in_text = self.links_in_text_XPATH
        # Extract 'other_items' 
