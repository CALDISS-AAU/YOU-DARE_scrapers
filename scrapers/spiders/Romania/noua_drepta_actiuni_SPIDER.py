...
class StaticSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    name = 'noua_dreapta_actiuni_SPIDER' # Spider name - must be unique within given project
    ...
    start_urls = ['https://www.nouadreapta.org/actiuni.html', 'https://www.nouadreapta.org/acasa.html'] # The url where the content to be scraped is found - can be multiple urls IF THE CSS/XPATH IS IDENTICAL!
    ...
    '''
    links_to_follow_XPATH = '//article/div/div/h2/a/@href'
    next_page_XPATH = '//ul/li[11]/a/@href'
    ...
    article_text_bits_XPATH = '//div[@itemprop="articleBody"]//text()'
    article_html_bits_XPATH = '//section[3]/div/div/div/section/div/div/div/div/div/article'
    article_categories = 'None'
    author_XPATH = '/html/head/meta[5]/text()'
    ...
    def open_spider(self, spider):
        ...
        self.existing_links = set(  # FIX: ensure this set always exists
            Static_Scrapy.load_existing_links(self.save_file, self.logger) or []
        )

    ### THE ACTUAL SPIDER FUNCTIONALITY ###
    def start_requests(self):
        """Generates paginated URLs based on `?start=X` pattern and optional max_pages."""
        base_url = "https://www.nouadreapta.org/actiuni.html"
        max_pages = self.MAX_PAGES if hasattr(self, "MAX_PAGES") and self.MAX_PAGES is not None else 43  # 42 pages × 10 = 420 articles
    ...
    def parse_front(self, response): # Can be renamed. IF IT IS REMEBER TO REDIRECT THE CALLBACK IN START_REQUESTS!
        ...
        self.logger.info(f"Processing page {current_page}: {response.url}")

        # FIX: define `links` before logging to prevent UnboundLocalError
        links = response.xpath(self.links_to_follow_XPATH).getall() or []
    ...
    def parse_article(self, response): # Can be renamed. IF IT IS REMEBER TO REDIRECT THE CALLBACK IN PARSE_FRONT!
        items = ScrapersItem()  # FIX: create a fresh item per request
        # Extract 'scrape_date'
        ...
        article_title_clean = General_Functions.clean_text(article_title) if article_title else None # Cleans the text - See doc string
        # Extract 'article_text'
        article_text_bits = response.xpath(self.article_text_bits_XPATH).getall() or []
        article_text_clean = General_Functions.join_and_clean(article_text_bits) if article_text_bits else None # Joins and cleans all text elements - See doc string
        article_HTML = response.text
        image_links = response.xpath(self.image_links_XPATH).getall() or []
        # Convert to absolute URLs
        image_links_full = [urljoin(response.url, src) for src in image_links]
        youtube_links = response.xpath(self.youtube_XPATH).getall() or []
        external_links = []
        if self.external_links_XPATH and self.external_links_XPATH.lower() != 'none':
            external_links = response.xpath(self.external_links_XPATH).getall() or []
