...
''' To run this spider pass the following to the terminal:
        ...
        scrapy crawl noua_dreapta_actiuni_acasa_SPIDER -a max_pages=x # MUST MATCH SPIDER NAME!
    ...
    OR
        ...
        mkdir -p ./data/Romania/noua_dreapta_actiuni_acasa_SPIDER # If the folder does not yet exist
        nohup scrapy crawl noua_dreapta_actiuni_acasa_SPIDER -a max_pages=1 > ./data/Romania/noua_dreapta_actiuni_acasa_SPIDER/noua_dreapta_actiuni_acasa_SPIDER_$(date +%F).log
'''
...
class StaticSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    name = 'noua_dreapta_actiuni_acasa_SPIDER' # Spider name - must be unique within given project
    ...
    start_urls = [
        'https://www.nouadreapta.org/actiuni.html', 
        'https://www.nouadreapta.org/acasa.html'
    ...
    '''
    links_to_follow_CSS = 'h2 > a::attr(href), h4.mfp_default_title > a::attr(href)'
    ...
    article_text_XPATH = '//div[@itemprop="articleBody"]//text()'
    image_links_XPATH = '//img/@src'
    embedded_media_links_XPATH = '//a[contains(@href, "youtube.com")]/@href'
    links_in_text_XPATH = '//div[@itemprop="articleBody"]//@href'
    other_items = None
    ...
    def parse_front(self, response): # Can be renamed. IF IT IS REMEBER TO REDIRECT THE CALLBACK IN START_REQUESTS!
        ...
        links =  response.css(self.links_to_follow_CSS).getall()
    ...
    def parse_article(self, response): # Can be renamed. IF IT IS REMEBER TO REDIRECT THE CALLBACK IN PARSE_FRONT!
        ...
        article_link_partial = response.meta['article_link']
        article_link = response.urljoin(article_link_partial)
        ...
        publication_date = self.publication_date_XPATH
        # Extract 'author' 
        author_clean = response.xpath(self.author_XPATH).get()
        ...
        # Extract 'image_links'
        image_links_partial = response.xpath(self.image_links_XPATH).getall()
        # Convert to absolute URLs
        image_links = [response.urljoin(src) for src in image_links_partial]
