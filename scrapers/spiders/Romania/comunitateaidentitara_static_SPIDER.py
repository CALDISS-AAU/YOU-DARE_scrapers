...
''' THIS SPIDER IS NOT ABLE TO RUN!
    To run this spider pass the following to the terminal:
        ...
        scrapy crawl comunitateaidentitara_static_SPIDER -a max_pages=x # MUST MATCH SPIDER NAME!
...
class StaticSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    name = 'comunitateaidentitara_static_SPIDER' # Spider name - must be unique within given project
    ...
    source_base = 'Comunitatea Identitara' # The base source name - each keyword will be appended to this
    search_template = 'https://comunitateaidentitara.com/category/{}'  # Template for keyword search URLs
    keywords = ['blog', 'actiuni', 'podcast']   # List of search keywords - customize this as needed
    ...
    author_CSS = ''
    article_text_bits_CSS = '.entry *::text'
    image_links_CSS = 'div.entry img::attr(src)'
    links_in_text_CSS = 'div.entry p a::attr(href), div.entry h1 a::attr(href)'
    article_HTML_bits_CSS = '.post-inner'
    author_CSS = ''
    article_categories_CSS = '.post-tag a::text'
    youtube_links_CSS = 'iframe::attr(src), video.wp-video-shortcode::attr(src), video.wp-video-shortcode source::attr(src), video.wp-video-shortcode a::attr(href)'
    # ... other queries
    ...
    def parse_article(self, response): # Can be renamed. IF IT IS REMEBER TO REDIRECT THE CALLBACK IN PARSE_FRONT!
        ...
        article_categories = response.css(self.article_categories_CSS).getall()
        embedded_med = response.css(self.youtube_links_CSS).getall()
        ...
        items['source'] = 'Comunitatea Identitara'
