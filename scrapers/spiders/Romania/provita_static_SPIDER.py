...
''' THIS SPIDER IS NOT ABLE TO RUN!
    To run this spider pass the following to the terminal:
        ...
        scrapy crawl provita_static_SPIDER -a max_pages=x # MUST MATCH SPIDER NAME!
...
class StaticSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    name = 'provita_static_SPIDER' # Spider name - must be unique within given project
    ...
    source_base = 'Asociatia Pro Vita' # The base source name - each keyword will be appended to this
    search_template = 'https://asociatiaprovita.ro/{}'  # Template for keyword search URLs
    keywords = ['blog', 'activitati/scrisori', 'activitati/analize-comentarii', 'activitati/relatii-internationale']   # List of search keywords - customize this as needed
    ...
    article_text_bits_CSS = '.entry-content :not(script)::text'
    ...
    article_categories_CSS = '.entry-categories a::text'
    article_tags_CSS = '.entry-tags a::text'
    # ... other queries
    ...
    def parse_article(self, response): # Can be renamed. IF IT IS REMEBER TO REDIRECT THE CALLBACK IN PARSE_FRONT!
        ...
        article_categories = response.css(self.article_categories_CSS).getall()
        article_tags = response.css(self.article_tags_CSS).getall()
        ...
        items['source'] = 'Asociatia Pro Vita'
        ...
        items['other_items'] = article_tags
