...
''' THIS SPIDER IS NOT ABLE TO RUN!
    To run this spider pass the following to the terminal:
        ...
        scrapy crawl il_redpillatore_SPIDER -a max_pages=x # MUST MATCH SPIDER NAME!
...
class StaticSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    name = 'redpillatore_IT_SPIDER' # Spider name - must be unique within given project
    ...
    start_urls = ['https://www.ilredpillatore.org/category/societa', 'https://www.ilredpillatore.org/category/societa/antifemminismo'] # The url where the content to be scraped is found - can be multiple urls IF THE CSS/XPATH IS IDENTICAL!
    ...
    media_links = '//article/div/figure[2]/video/@src'
    ...
    def parse_article(self, response): # Can be renamed. IF IT IS REMEBER TO REDIRECT THE CALLBACK IN PARSE_FRONT!
        ...
        article_html = response.xpath(self.article_HTML_bits_XPATH).get()
        # Extract 'author'
        ...
        article_text_clean = General_Functions.join_and_clean([
            t.strip() for t in article_text_bits if t.strip()
        ]) # Joins and cleans all text elements - See doc string
        ...
        # Extract media_links
        embedded_media_links = response.xpath(self.media_links).getall()
        # Extract links in text
        links_in_text = response.xpath(self.article_links_XPATH).getall()
