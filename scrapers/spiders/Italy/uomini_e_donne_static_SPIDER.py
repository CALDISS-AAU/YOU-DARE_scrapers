...
''' To run this spider pass the following to the terminal:
        ...
        scrapy crawl uomini_e_donne_SPIDER -a max_pages=x # MUST MATCH SPIDER NAME!
    ...
    OR
        ...
        mkdir -p ./data/Italy/uomini_e_donne_SPIDER # If the folder does not yet exist
        nohup scrapy crawl uomini_e_donne_SPIDER -a max_pages=1 > ./data/Italy/uomini_e_donne_SPIDER/uomini_e_donne_static_SPIDER_$(date +%F).log
'''
...
class StaticSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    name = 'uomini_e_donne_SPIDER' # Spider name - must be unique within given project
    ...
    source = 'Uomini e donne' # The source of the articles - NOT the author!
    start_urls = ['https://www.uominibeta.org/category/articoli/'] # The url where the content to be scraped is found - can be multiple urls IF THE CSS/XPATH IS IDENTICAL!
    ...
    '''
    links_to_follow_CSS = '.entry-title a::attr(href)'
    next_page_css = '.pag-left a::attr(href)'
    ...
    publication_date_xpath = 'substring-before(//span[@class="meta"]/text()[1], "|")'
    author_CSS = '.entry-author *::text'
    article_categories_CSS = None
    article_text_xpath = '//div[@class="entry-content"]//div[@dir="auto"]//text() | //div[@class="entry-content"]//p//text() | //div[@class="entry-content"]//ol//text()'
    image_links_CSS = '.entry-content img::attr(src)'
    embedded_media_links_CSS = None
    links_in_text_XPATH = '//div[@class="entry-content"]//div[@dir="auto"]//@href | //div[@class="entry-content"]//p//@href | //div[@class="entry-content"]//ol//@href'
    other_items = None
    ...
    def parse_article(self, response): # Can be renamed. IF IT IS REMEBER TO REDIRECT THE CALLBACK IN PARSE_FRONT!
        ...
        publication_date = response.xpath(self.publication_date_xpath).get().strip()
        # Extract 'author'
