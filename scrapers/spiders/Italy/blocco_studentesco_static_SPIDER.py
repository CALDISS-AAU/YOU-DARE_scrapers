...
''' To run this spider pass the following to the terminal:
    ...
    OR
        ...
        mkdir -p ./data/Italy/blocco_studentesco_SPIDER # If the folder does not yet exist
        nohup scrapy crawl blocco_studentesco_SPIDER -a max_pages=1 > ./data/Italy/blocco_studentesco_SPIDER/blocco_studentesco_SPIDER_$(date +%F).log
'''
...
class StaticSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    ...
    source = 'Blocco Studentesco' # The base source name
    start_urls = [
        'https://www.bloccostudentesco.org/?s=genere',
        'https://www.bloccostudentesco.org/?s=femminismo',
        'https://www.bloccostudentesco.org/?s=LGBT',
        'https://www.bloccostudentesco.org/?s=aborto',
        'https://www.bloccostudentesco.org/?s=virilita'
    ]
