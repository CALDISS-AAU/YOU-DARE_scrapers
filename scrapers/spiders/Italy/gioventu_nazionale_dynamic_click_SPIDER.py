...
''' To run this spider pass the following to the terminal:
        ...
        scrapy crawl gioventu_nazionale_SPIDER -a max_clicks=x # MUST MATCH SPIDER NAME!
    where -a max_clicks=x is an optional parameter to limit the number of clicks to render more articles on the front page
    OR
        cd ./path/to/YOU-DARE_scrapers-folder
        mkdir -p ./data/Italy/gioventu_nazionale_SPIDER # If the folder does not yet exist
        nohup scrapy crawl gioventu_nazionale_SPIDER -a max_clicks=0 > ./data/Italy/gioventu_nazionale_SPIDER/gioventu_nazionale_SPIDER_$(date +%F).log
'''
...
class DynamicSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    ...
    click_button_selector_STOP_CSS = '.paginator .paginator-more-button.hidden' # If this is not relevant, set this query to None
    ...
    article_text_CSS = 'div.entry-content :not(script)::text'
