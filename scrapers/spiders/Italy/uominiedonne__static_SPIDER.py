...
''' THIS SPIDER IS NOT ABLE TO RUN!
    To run this spider pass the following to the terminal:
        ...
        scrapy crawl uominiedonne_static_SPIDER -a max_pages=x # MUST MATCH SPIDER NAME!
...
class StaticSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    name = 'uominiedonne_static_SPIDER' # Spider name - must be unique within given project
    ...
    article_text_bits_xpath = '//div[@class="entry-content"]//div[@dir="auto"]//text() | //div[@class="entry-content"]//p//text() | //div[@class="entry-content"]//ol//text()'
    ...
    def parse_article(self, response): # Can be renamed. IF IT IS REMEBER TO REDIRECT THE CALLBACK IN PARSE_FRONT!
        ...
        article_author = response.css(self.author_CSS).get()
