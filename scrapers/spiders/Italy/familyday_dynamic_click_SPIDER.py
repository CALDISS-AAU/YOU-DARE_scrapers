...
''' To run this spider pass the following to the terminal:
        ...
        scrapy crawl familyday_SPIDER -a max_clicks=x # MUST MATCH SPIDER NAME!
    where -a max_clicks=x is an optional parameter to limit the number of clicks to render more articles on the front page
    OR
        cd ./path/to/YOU-DARE_scrapers-folder
        mkdir -p ./data/Italy/familyday_SPIDER # If the folder does not yet exist
        nohup scrapy crawl familyday_SPIDER -a max_clicks=0 > ./data/Italy/familyday_SPIDER/familyday_SPIDER_$(date +%F).log
'''
...
class DynamicSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    name = 'familyday_SPIDER' # Spider name - must be unique within given project
    ...
    article_text_XPATH = '//div[contains(@class, "et_pb_text_inner")]//h5/text() | //div[contains(@class, "et_pb_post_content_0_tb_body")]//*[not(ancestor::div[contains(@class, "et_social_inline")])]/text()' # Previous : '.et_pb_text_inner h5::text, .et_pb_post_content_0_tb_body :not(.et_social_inline)::text'
    ...
    links_in_text_CSS = '.et_pb_post_content_0_tb_body p a::attr(href)'
    other_items = None
    ...
    def parse(self, response): # Can't be renamed
        ...
        links = yield deferToThread(
            ...
            Dynamic_Click_Scrapy.click_and_collect_links(
                ...
                # pagination_navigates = True, # default value = False, set to true if the url changes when clicking the button to render new articles
