...
''' To run this spider pass the following to the terminal:
        ...
        scrapy crawl template_scroll_SPIDER -a max_scrolls=x # MUST MATCH SPIDER NAME!
    where -a max_scrolls=x is an optional parameter to limit the number of scrolls to render more articles on the front page
    OR
        cd ./path/to/YOU-DARE_scrapers-folder
        mkdir -p ./data/Template/template_scroll_SPIDER # If the folder does not yet exist
        nohup scrapy crawl template_scroll_SPIDER -a max_scrolls=0 > ./data/Template/template_scroll_SPIDER/template_scroll_SPIDER_$(date +%F).log
'''
...
class DynamicSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    name = 'template_scroll_SPIDER' # Spider name - must be unique within given project
    ...
    start_urls = ['https://quotes.toscrape.com/scroll'] # The url where the content to be scraped is found - can be multiple urls IF THE CSS/XPATH IS IDENTICAL!
