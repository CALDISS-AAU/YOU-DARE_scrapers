...
''' THIS SPIDER IS NOT ABLE TO RUN!
    To run this spider pass the following to the terminal:
        ...
        scrapy crawl motstandsrorelsen_SPIDER -a max_pages=x # MUST MATCH SPIDER NAME!
...
class DynamicSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    name = 'motstandsrorelsen_SPIDER' # Spider name - must be unique within given project
    ...
    source = 'Motstandsrorelsen' # The source of the articles - NOT the author!
    start_urls = ['https://xn--motstndsrrelsen-llb70a.se/'] # The url where the content to be scraped is found - can be multiple urls IF THE CSS/XPATH IS IDENTICAL!
    ...
    '''
    links_to_follow_CSS = '.flow.article-flow.marginfix-fullw article a::attr(href), .flow.article-flow article a::attr(href)'
    click_button_selector_CSS = '.loadmore-inner' # CSS selector for the "Load more" button
    click_button_selector_STOP_CSS = ''
    
    # CSS for last "next page"-button (it still has a link but is disabled so it would keep loading the same articles on the last page indefinitely)
    ...
    '''
    article_title_CSS = '.title h1::text, h1.wp-block-heading::text'
    author_CSS = '.name::text'
    publication_date_CSS = '.title time::attr(datetime)'
    article_text_bits_CSS = '.entry-content p *::text, .entry-content h2::text, .entry-content h2 *::text, .fakta-full *::text'
    image_links_CSS = '.wp-block-image img::attr(src)'
    article_HTML_bits_CSS = 'div.entry-content p'
    ...
    def parse(self, response): # Can't be renamed
        ...
        #STEP 1 = Getting the response URL
        url = response.url
        # STEP 2 = Collecting all article links by clicking the "loadmore" button
        links = yield deferToThread(
            asyncio.run,
            Dynamic_Scrapy_Click.fetch_links_with_clicking_stop_at_certain_article(
                url,
                ...
                wait_time=2000,
                stop_if_url_starts_with ='2021/06/01'
            )
        )
        # STEP 3 = We join the links into a list
