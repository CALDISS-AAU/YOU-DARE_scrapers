...
''' To run this spider pass the following to the terminal:
        ...
        scrapy crawl flashback_integration_och_invandring_SPIDER -a max_pages_posts=x # MUST MATCH SPIDER NAME!
    where -a max_pages=x is an optional parameter
    OR
        cd ./YOU-DARE/scrapers
        mkdir -p /work/YOU-DARE/scrapers/data/Sweden/flashback_integration_och_invandring_SPIDER # If the folder does not yet exist
        nohup scrapy crawl flashback_integration_och_invandring_SPIDER > /work/YOU-DARE/scrapers/data/Sweden/flashback_integration_och_invandring_SPIDER/flashback_integration_och_invandring_SPIDER_2025-10-10_SPIDER.log
'''

# # Adjust waittime before trying to scrape new page again (60s)
# class FixedDelayRetryMiddleware(RetryMiddleware):
#     """Waits 60s before each retry. Only active for spiders that enable it via custom_settings."""
#     def _retry(self, request, reason, spider):
#         retries = request.meta.get('retry_times', 0)
#         if retries > 0:
#             spider.logger.info(f"[{spider.name}] Waiting 60s before retry {retries+1} due to {reason}")
#             time.sleep(60)  # blocks this process for 60s; simple and effective
...
class StaticSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    name = 'flashback_integration_och_invandring_SPIDER' # Spider name - must be unique within given project
    ...
    source = 'Flashback - integration och invandring' # The source of the articles - NOT the author!
    start_urls = ['https://www.flashback.org/f226-integration-och-invandring-51007']#['https://www.flashback.org/f226-integration-och-invandring-50822'] # The url where the content to be scraped is found - can be multiple urls IF THE CSS/XPATH IS IDENTICAL!
    ...
    #         __name__ + '.FixedDelayRetryMiddleware': 550,  # uses the class defined above in THIS file
    #     },
    # }
    custom_settings = {
        'AUTOTHROTTLE_ENABLED': True,
