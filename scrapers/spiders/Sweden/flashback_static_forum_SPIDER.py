...
''' To run this spider pass the following to the terminal:
        ...
        scrapy crawl flashback_SPIDER -a max_pages_posts=x # MUST MATCH SPIDER NAME!
    ...
    OR
        cd ./path/to/YOU-DARE_scrapers_folder
        mkdir -p ./data/Sweden/flashback_SPIDER # If the folder does not yet exist
        nohup scrapy crawl flashback_SPIDER -a max_pages_posts=1 > ./data/Sweden/flashback_SPIDER/flashback_SPIDER_$(date +%F).log
'''
...
class StaticSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    name = 'flashback_SPIDER' # Spider name - must be unique within given project
