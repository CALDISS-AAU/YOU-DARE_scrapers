...
''' To run this spider pass the following to the terminal:
        ...
        scrapy crawl flashback_censur_och_yttrandefrihet_SPIDER -a max_pages_posts=x # MUST MATCH SPIDER NAME!
    where -a max_pages=x is an optional parameter
    OR
        cd ./YOU-DARE/scrapers
        mkdir -p /work/YOU-DARE/scrapers/data/Sweden/flashback_censur_och_yttrandefrihet_SPIDER # If the folder does not yet exist
        nohup scrapy crawl flashback_censur_och_yttrandefrihet_SPIDER > /work/YOU-DARE/scrapers/data/Sweden/flashback_censur_och_yttrandefrihet_SPIDER/flashback_censur_och_yttrandefrihet_SPIDER_2025-10-08_SPIDER.log
'''
...
class StaticSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    name = 'flashback_censur_och_yttrandefrihet_SPIDER' # Spider name - must be unique within given project
    ...
    source = 'Flashback - censur och yttrandefrihet' # The source of the articles - NOT the author!
    start_urls = ['https://www.flashback.org/f69-censur-och-yttrandefrihet-51006'] # The url where the content to be scraped is found - can be multiple urls IF THE CSS/XPATH IS IDENTICAL!
