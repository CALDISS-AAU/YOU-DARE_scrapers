...
#     name = 'samnytt_debatt_SPIDER' # Spider name - must be unique within given project
...
#     source_base = 'Samnytt' # The base source name - each keyword will be appended to this
#     search_template = 'https://samnytt.se/category/{}'  # Template for keyword search URLs
#     keywords = ['debatt']#['ledare', 'debatt', 'kronikor']  # List of search keywords - customize this as needed
...
#         //a/@href
#         '''.strip()
#         # '//div[contains(@class, "post-content")]//p[not(starts-with(normalize-space(.), "LÄS ÄVEN: "))]//a/@href'
...
#         links = [link if link.startswith('http') else f'https://samnytt.nu{link}' for link in raw_links]
...
class StaticSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    ...
    source_base = 'Samnytt' # The base source name - each keyword will be appended to this
    search_template = 'https://samnytt.se/category/{}'  # Template for keyword search URLs
    keywords = ['ledare', 'debatt', 'kronikor']  # List of search keywords - customize this as needed
    ...
    def parse_front(self, response): # Can be renamed. IF IT IS REMEBER TO REDIRECT THE CALLBACK IN START_REQUESTS!
        ...
        for href in raw_links:
            ...
            if path not in path_seen:
                path_seen.add(path)
                full_url = urljoin('https://samnytt.se', path)  # Prefer .se domain
