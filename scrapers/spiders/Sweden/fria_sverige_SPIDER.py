...
''' To run this spider pass the following to the terminal:
        ...
        scrapy crawl fria_sverige_SPIDER -a max_scrolls=x # MUST MATCH SPIDER NAME!
    where -a max_scrolls=x is an optional parameter
    OR
        cd ./YOU-DARE/scrapers
        mkdir -p /work/YOU-DARE/scrapers/data/Sweden/fria_sverige_SPIDER # If the folder does not yet exist
        nohup scrapy crawl fria_sverige_SPIDER > /work/YOU-DARE/scrapers/data/Sweden/fria_sverige_SPIDER/fria_sverige_SPIDER_2025-09-23_SPIDER.log
'''

### SPIDER ###
class DynamicSpider(scrapy.Spider):
    name = 'fria_sverige_SPIDER'
    region = 'Sweden'
    source = 'Fria Sverige'
    start_urls = ['https://forum.friasvenskar.se/c/allmant/4']

    links_to_follow_CSS = 'a.title::attr(href)'

    article_title_CSS = 'h1 *::text'
    author_CSS = 'span.first *::text'
    publication_date_CSS = '.post-date span.relative-date::attr(title)'
