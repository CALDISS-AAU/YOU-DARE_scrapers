...
''' To run this spider pass the following to the terminal:
        ...
        scrapy crawl nordfront_SWEDEN_SPIDER -a max_clicks=x # MUST MATCH SPIDER NAME!
    where -a max_clicks=x is an optional parameter to limit the number of clicks to render more articles on the front page
    OR
        cd ./path/to/YOU-DARE_scrapers-folder
        mkdir -p ./data/Sweden/nordfront_SWEDEN_SPIDER # If the folder does not yet exist
        nohup scrapy crawl nordfront_SWEDEN_SPIDER -a max_clicks=0 > ./data/Sweden/nordfront_SWEDEN_SPIDER/nordfront_SWEDEN_SPIDER_$(date +%F).log
'''
...
class DynamicSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    name = 'nordfront_SWEDEN_SPIDER' # Spider name - must be unique within given project
    ...
    click_button_selector_STOP_CSS = None # If this is not relevant, set this query to None
    ...
    article_text_XPATH = '//div[@class="text"]//text()'
    image_links_XPATH = '//figure[@class="complex-media"]//img/@mcpp_source_src'
    embedded_media_links_XPATH = '//article//iframe[contains(@src, "youtube.com")]/@src'
    links_in_text_XPATH = '//article[@id="main-article"]//div[@class="text"]//a[@href]/@href'
    other_items = None
    # .smr queries
    ...
    smr_article_text_CSS = '.post-content *::text'
    smr_image_links_XPATH = '//article/span/p/img/@src'
    smr_embedded_media_links_XPATH = '//article//iframe[contains(@src, "youtube.com")]/@src'
    smr_links_in_text_XPATH = '//article/span/p/a/@href'
    ...
    def parse(self, response): # Can't be renamed
        ...
        links = yield deferToThread(
            ...
            Dynamic_Click_Scrapy.click_and_collect_links(
                ...
                wait_time=10000, # default value = 2000, increase if needed
                ...
                incremental_retries=50 # default value = 5, increase if needed
            )
        )
    ...
    def parse_article(self, response, article_link): # Can be renamed. IF IT IS REMEBER TO REDIRECT THE CALLBACK IN PARSE!
        ...
        if is_smr:
            ...
            article_title = response.xpath(self.smr_article_title_XPATH).get() # .get() returns only the first element. Use .getall() to return a list of all elements if more than one element is expected
            ...
            publication_date_raw = response.xpath(self.smr_publication_date_XPATH).get()
            publication_date = publication_date_raw.strip() if publication_date_raw else None
            # Extract 'author' 
            ...
            article_categories = response.xpath(self.smr_article_categories_XPATH).getall()
            # Extract 'article_text'
            article_text_bits = response.css(self.smr_article_text_CSS).getall() 
            article_text_clean = General_Functions.join_and_clean(article_text_bits) # Joins and cleans all text elements - See doc string
            # Extract 'image_links' 
            image_links = response.xpath(self.smr_image_links_XPATH).getall()
            # Extract 'embedded_media_links' 
            embedded_media_links = response.xpath(self.smr_embedded_media_links_XPATH).getall()
            # Extract 'links_in_text' 
            links_in_text = response.xpath(self.smr_links_in_text_XPATH).getall()
        else:
            # Extract 'article_title'
            ...
            if article_title: # Only cleans existing articles
                ...
                article_title_clean = article_title
            # Extract 'publication_date'
            publication_date_raw = response.xpath(self.publication_date_XPATH).get()
            publication_date = publication_date_raw.strip() if publication_date_raw else None
            # Extract 'author' 
