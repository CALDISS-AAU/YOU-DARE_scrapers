...
class StaticSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    ...
    article_text_bits_CSS = 'div.oct-post-content p *::text'
    image_links_CSS = '.oct-post-content img::attr(src)'
    external_links_CSS = 'div.oct-post-content a::attr(href)'
    article_HTML_bits_CSS = 'div.oct-post-content p'
    # ... other queries
    ...
    def parse_article(self, response): # Can be renamed. IF IT IS REMEBER TO REDIRECT THE CALLBACK IN PARSE_FRONT!
        ...
        # Extract 'author'
        author_raw  = article.css(self.author_CSS).getall()
        author_clean = [a.strip() for a in author_raw if a.strip()]
        author = author_clean[0] if author_clean else None
        ...
        article_HTML = ' '.join(article_HTML_bits).strip() # Joins and cleans all text elements - See doc string
