...
''' To run this spider pass the following to the terminal:
        ...
        scrapy crawl identitaer_SPIDER -a max_pages=x # MUST MATCH SPIDER NAME!
    ...
    OR
        ...
        mkdir -p ./data/Denmark/identitaer_SPIDER # If the folder does not yet exist
        nohup scrapy crawl identitaer_SPIDER -a max_pages=1 > ./data/Denmark/identitaer_SPIDER/identitaer_SPIDER_$(date +%F).log
'''
...
class StaticSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    name = 'identitaer_SPIDER' # Spider name - must be unique within given project
    ...
    source = 'Identitaer' # The source of the articles - NOT the author!
    start_urls = ['https://identitaer.dk/category/presse'] # The url where the content to be scraped is found - can be multiple urls IF THE CSS/XPATH IS IDENTICAL!
    ...
    '''
    links_to_follow_CSS = 'a.tt-post-title::attr(href)'
    ...
    publication_date_CSS = '.tt-post-date-single::text'
    author_CSS = None 
    article_categories_CSS = None
    article_text_CSS = '#content-wrapper>.container p *::text, #content-wrapper>.container h2 *::text'
    image_links_CSS = 'figure img::attr(src)'
    embedded_media_links_CSS = 'iframe[src]:not([src="about:blank"])::attr(src)'
    links_in_text_CSS = '#content-wrapper>.container p a::attr(href), #content-wrapper>.container h2 a::attr(href)'
    other_items = None
