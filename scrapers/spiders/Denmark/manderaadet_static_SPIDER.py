...
''' To run this spider pass the following to the terminal:
        ...
        scrapy crawl manderaadet_SPIDER -a max_pages=x # MUST MATCH SPIDER NAME!
    ...
    OR
        ...
        mkdir -p ./data/Denmark/manderaadet_SPIDER # If the folder does not yet exist
        nohup scrapy crawl manderaadet_SPIDER -a max_pages=1 > ./data/Denmark/manderaadet_SPIDER/manderaadet_SPIDER_$(date +%F).log
'''
...
class StaticSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    name = 'manderaadet_SPIDER' # Spider name - must be unique within given project
    ...
    source = 'Manderaadet' # The source of the articles - NOT the author!
    start_urls = ['https://manderaadet.dk/nyt-fra-manderadet/'] # The url where the content to be scraped is found - can be multiple urls IF THE CSS/XPATH IS IDENTICAL!
    ...
    '''
    links_to_follow_CSS = 'a.btn-primary::attr(href)'
    next_page_CSS = 'nav .next::attr(href)'
    # FROM THE ARTICLE PAGE!!!
    ''' CSS or XPath queries for relevant information found on the individual article pages.
    '''
    article_CSS = 'article'
    article_title_CSS = 'h1::text'
    publication_date_CSS = '.post-date.entry-date.published.updated::text'
    author_CSS = 'li.list-inline-item.post-author *::text'
    article_categories_CSS = None
    article_text_CSS = 'div.oct-post-content p *::text'
    image_links_CSS = '.oct-post-content img::attr(src)'
    embedded_media_links_CSS = None
    links_in_text_CSS = 'div.oct-post-content a::attr(href)'
    other_items = None
    ...
    def parse_article(self, response): # Can be renamed. IF IT IS REMEBER TO REDIRECT THE CALLBACK IN PARSE_FRONT!
        ...
        publication_date = article.css(self.publication_date_CSS).get().strip()
        # Extract 'author'
        author  = article.css(self.author_CSS).getall()
        if author:
