...
''' To run this spider pass the following to the terminal:
        ...
        scrapy crawl reel_ligestilling_SPIDER -a max_clicks=x # MUST MATCH SPIDER NAME!
    where -a max_clicks=x is an optional parameter to limit the number of clicks to render more articles on the front page
    OR
        cd ./path/to/YOU-DARE_scrapers-folder
        mkdir -p ./data/Denmark/reel_ligestilling_SPIDER # If the folder does not yet exist
        nohup scrapy crawl reel_ligestilling_SPIDER -a max_clicks=0 > ./data/Denmark/reel_ligestilling_SPIDER/reel_ligestilling_SPIDER_$(date +%F).log
'''
...
class DynamicSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    name = 'reel_ligestilling_SPIDER' # Spider name - must be unique within given project
    ...
    source = 'Reel ligestilling' # The source of the articles - NOT the author!
    start_urls = ['https://reelligestilling.dk/category/indhold/ligestillingsnyt/'] # The url where the content to be scraped is found - can be multiple urls IF THE CSS/XPATH IS IDENTICAL!
    ...
    '''
    links_to_follow_CSS = '.td-module-thumb a::attr(href)'
    click_button_selector_CSS = 'a.td-ajax-next-page' # CSS selector for the "Load more" button
    click_button_selector_STOP_CSS = 'ajax-page-disabled' # If this is not relevant, set this query to None
    # FROM THE ARTICLE PAGE!!!
    ''' CSS or XPath queries for relevant information found on the individual article pages.
    '''
    article_title_CSS = 'h1.tdb-title-text::text'
    publication_date_CSS = '.entry-date *::text'
    author_CSS = '.tdb-author-name::text'
    article_categories_CSS = None
    article_text_CSS = '.tdb-block-inner p *::text, .tdb-block-inner h3 *::text, .tdb-block-inner ul *::text, .tdb-block-inner h4 *::text'
    image_links_CSS = 'figure img::attr(src)'
    embedded_media_links_CSS = 'iframe::attr(src)'
    links_in_text_CSS = '.tdb-block-inner p *::attr(href)'
    other_items = None
