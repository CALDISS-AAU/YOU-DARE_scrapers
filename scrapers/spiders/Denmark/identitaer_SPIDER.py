...
class StaticSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    ...
    article_text_bits_CSS = '#content-wrapper>.container p *::text'
    external_links_CSS = '#content-wrapper>.container p a::attr(href)'
    image_links_CSS = 'figure img::attr(src)'
    article_HTML_bits_CSS = '#content-wrapper>.container p'
    youtube_links_CSS = 'iframe[src]:not([src="about:blank"])::attr(src)'
    ...
    def parse_article(self, response): # Can be renamed. IF IT IS REMEBER TO REDIRECT THE CALLBACK IN PARSE_FRONT!
        ...
        youtube_links = response.css(self.youtube_links_CSS).getall()
