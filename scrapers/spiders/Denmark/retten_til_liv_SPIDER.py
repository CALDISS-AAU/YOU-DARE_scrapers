...
''' THIS SPIDER IS NOT ABLE TO RUN!
    To run this spider pass the following to the terminal:
        ...
        scrapy crawl retten_til_liv_SPIDER -a max_pages=x # MUST MATCH SPIDER NAME!
...
class DynamicSpider(scrapy.Spider): # Can be changed but it's not necessary - if changed also change Super in from_crawler function
    ...
    '''
    article_CSS = 'div[itemprop="articleBody"] .col-xs-12'
    ...
    author_CSS = 'li.the_author::text'
    article_text_bits_CSS = 'div.col-sm-11.col-sm-push-1.content p *::text'
    image_links_CSS = '#article img::attr(src)'
    image_captions_CSS = '#article figcaption::text'
    external_links_CSS = '.external *::attr(href)'
    article_HTML_bits_CSS = 'div[itemprop="articleBody"]'
    category_css = '.category.span.tax-title::text'
    ...
    def parse(self, response): # Can't be renamed
        ...
        for article in articles:
            ...
            article_link = article.css(self.links_to_follow_CSS).get()
    ...
    def parse_article(self, response, article_link): # Can be renamed. IF IT IS REMEBER TO REDIRECT THE CALLBACK IN PARSE!
        ...
        # Extract 'author' 
        raw_author = response.css(self.author_CSS).getall()
        author_text = ' '.join(raw_author).strip()
        if author_text: # Only cleans existing articles
            author_clean = General_Functions.clean_text(author_text) # Cleans the text - See doc string
            author_clean = re.sub(r'^\s*af\s+', '', author_clean, flags=re.IGNORECASE)
        else:
            author_clean = 'Kerstin Hoffmann'
