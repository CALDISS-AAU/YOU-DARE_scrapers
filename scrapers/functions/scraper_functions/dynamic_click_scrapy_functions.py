...
class Dynamic_Click_Scrapy:
    ...
    @staticmethod
    async def click_and_collect_links_and_article_categories(
        url: str,
        click_button_selector: str,
        container_selector: str,
        link_selector: str,
        article_categories_selector: str,
        max_clicks: Optional[int] = None,
        wait_time: int = 2000,
        stop_when_button_has_class: Optional[str] = None,
        *,
        ...
        - For every element matching `container_selector`, it extracts:
            * a link from `link_selector` (href normalized to an absolute URL),
            * a publication date from `article_categories_selector` (text, stripped).
        - Results are deduplicated by link.
        - The loop stops when:
            * `max_clicks` is reached,
            * no button is found or the button becomes hidden,
        ...
        Args:
            ...
            container_selector (str): Selector for the outer container holding an item
                (e.g., an article card).
            link_selector (str): Selector (relative to each container) for the link element.
            article_categories_selector (str): Selector (relative to each container) for the
        ...
        """
        if not container_selector or not link_selector or not article_categories_selector:
            raise ValueError("container_selector, link_selector and article_categories_selector are all required.")
            ...
            async def collect_entries_into_list() -> int:
                ...
                for i in range(n):
                    ...
                    try:
                        ...
                        # Article categories
                        category_loc = _loc(c, article_categories_selector).first
                        article_categories = None
                        if await category_loc.count() > 0:
                            try:
                                txt = await category_loc.inner_text()
                                article_categories = txt.strip() if txt is not None else None
                            except Exception:
                                article_categories = None
